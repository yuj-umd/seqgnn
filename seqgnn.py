...
class SeqGNN(nn.Module):
    def __init__(self, num_layers, num_mlp_layers, input_dim, hidden_dim, output_dim,
        ...
        # a list of SRNs/GRUs/LSTMs
        self.seqgnn_rnns = torch.nn.ModuleList()

        # for layer in range(self.num_layers-1):
        for layer in range(self.num_layers):
            if layer == 0:
                # if self.neighbor_pooling_type == "srn":
                #     self.seqgnn_rnns.append(nn.RNN(input_dim+1, hidden_dim, num_layers=num_mlp_layers, batch_first=True))
                # elif self.neighbor_pooling_type == "gru":
                #     self.seqgnn_rnns.append(nn.GRU(input_dim+1, hidden_dim, num_layers=num_mlp_layers, batch_first=True))
                # elif self.neighbor_pooling_type == "lstm":
                # self.seqgnn_rnns.append(nn.LSTM(input_dim+1, hidden_dim, num_layers=num_mlp_layers, batch_first=True))
                self.seqgnn_rnns.append(nn.LSTM(input_dim+1, hidden_dim, num_layers=1, batch_first=False))
                # else:
                #     pass
            else:
                # if self.neighbor_pooling_type == "srn":
                #     self.seqgnn_rnns.append(nn.RNN(hidden_dim+1, hidden_dim, num_layers=num_mlp_layers, batch_first=True))
                # elif self.neighbor_pooling_type == "gru":
                #     self.seqgnn_rnns.append(nn.GRU(hidden_dim+1, hidden_dim, num_layers=num_mlp_layers, batch_first=True))
                # elif self.neighbor_pooling_type == "lstm":
                # self.seqgnn_rnns.append(nn.LSTM(hidden_dim+1, hidden_dim, num_layers=num_mlp_layers, batch_first=True))
                self.seqgnn_rnns.append(nn.LSTM(hidden_dim+1, hidden_dim, num_layers=1, batch_first=False))
                # else:
                #     pass
        ...
        for layer in range(self.num_layers):
            if layer == 0 and self.graph_pooling_type != "seqgnn":
    ...
    def seqgnn_pool(self, batch_graph, h, layer=0):
        max_num_nodes = max([len(graph.g) for graph in batch_graph])
        seqgnn_rnn_model = self.seqgnn_rnns[layer]
        num_graphs = len(batch_graph)
        start_idx = [0]
        h_seqgnn = torch.zeros(num_graphs, self.hidden_dim).to(self.device)
        for i, graph in enumerate(batch_graph):
            start_idx.append(start_idx[i] + len(graph.g))
        for i in range(num_graphs):
            g = batch_graph[i].g
            num_nodes = len(g)
            permutations = batch_graph[i].permutations
            sequence_edges = batch_graph[i].sequence_edges
            h_cur = h[start_idx[i]:start_idx[i+1], :]
            h_i = torch.zeros(batch_graph[i].num_sequence_samples, self.hidden_dim).to(self.device)
            for j in range(batch_graph[i].num_sequence_samples):
                permutation = torch.LongTensor(permutations[j]).to(self.device)
                sequence_edge = sequence_edges[j].to(self.device)
                h_cur_perm = h_cur[permutation, :]
                h_cur_perm_edge = torch.cat((h_cur_perm, sequence_edge), 1)
                h_cur_perm_edge = h_cur_perm_edge.view(num_nodes, 1, -1)
                output, (hn, cn) = seqgnn_rnn_model(h_cur_perm_edge)
                h_i[j, :] = hn.view(1, self.hidden_dim)
            h_seqgnn[i, :] = torch.mean(h_i, 0)
        return h_seqgnn
    ...
    def forward(self, batch_graph):
        ...
        for layer, h in enumerate(hidden_rep):
            if self.graph_pooling_type == "none":
                pooled_h = h
            elif self.graph_pooling_type == "seqgnn":
                pooled_h = self.seqgnn_pool(batch_graph, h, layer)
            else:
