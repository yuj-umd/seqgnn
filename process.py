...
class S2VGraph(object):
    ...
    def get_random_permutation(self):
        ...
        for i in range(n-1):
            ...
            if len(neighbors_filtered) > 0:
                next = random.choice(neighbors_filtered)
            else:
    ...
    def permutation_to_sequence(self, permutation):
        g = self.g
        n = len(g)
        permutation = torch.LongTensor(permutation)
        permutation_features = self.node_features[permutation, :]
        edge_features = torch.zeros(n, 1)
        for i in range(n-1):
            if permutation[i+1] in self.neighbors[permutation[i]]:
                edge_features[i, 0] = 1
            else:
                edge_features[i, 0] = 0 
        edge_features[n-1, 0] = 0
        
        sequence_features = torch.cat((permutation_features, edge_features), 1)
        return sequence_features, edge_features

    def get_sequence_samples(self, num_sequence_samples=1):
        if num_sequence_samples == None:
            num_sequence_samples = len(self.g)
            self.num_sequence_samples = num_sequence_samples
        elif num_sequence_samples == 1:
            self.num_sequence_samples = num_sequence_samples
            permutation = list(range(len(self.g)))
            self.permutations.append(permutation)
            sequence_features, edge_features = self.permutation_to_sequence(permutation)
            self.sequences.append(sequence_features)
            self.sequence_edges.append(edge_features)
            return 
        
        for i in range(num_sequence_samples):
