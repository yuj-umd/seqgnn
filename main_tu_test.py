...
from models.seqgnn import SeqGNN
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default="IMDBBINARY_test",
    ...
    # parser.add_argument('--graph_pooling_type', type=str, default="sum", choices=["sum", "average", "seqgnn"],
    #                     help='pooling for all nodes in a graph: sum or average')
    parser.add_argument('--graph_pooling_type', type=str, default="seqgnn", choices=["sum", "average", "seqgnn"],
                        help='pooling for all nodes in a graph: sum or average')
    ...
    parser.add_argument('--neighbor_pooling_type', type=str, default="average", choices=["sum", "average", "max", "srn", "gru", "lstm"],
                        help='pooling for neighboring nodes: sum, average, max, srn, gru, or lstm')
    ...
    model = SeqGNN(args.num_layers, args.num_mlp_layers, train_graphs[0].node_features.shape[1], args.hidden_dim,
                     num_classes, args.final_dropout, args.graph_pooling_type, args.neighbor_pooling_type, device).to(device)
